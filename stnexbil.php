...
mysqli_select_db($con,'books');
	$help= $_POST['help'];
	$bily=$help+1;
	$ss3="select stpname from stock where stbill='$bily'";
	...
	$ss="select * from stock where stbill='$bily'";
	$res=mysqli_query($con,$ss);
	$re=mysqli_fetch_row($res);
	$srena=$re[1];
	$srdat=$re[2];
	$srnar=$re[4];
	$ss1="select MAX(stbill) from stock";
		...
		.saledel
{
			...
			font-size: 20px; 
			width: 150px;
			box-shadow: 10px 25px 50px 10px black;
			z-index: 5;
}
	</style>
</head>
...
<form method="post" action="stockphp.php">
<label class="l"> STOCK OPEN:</label>
<input type="textbox" name="cname" class="sname" id="sname"  required>
	...
	<table class="table" id='table'>
	  	<thread class="thread">
			<tr>
				...
				<th>Amount</th>
		<?php
			$da="select * from stock where stbill='$bily' ";
...
<form method="post" action="stpre.php" name="f1" >
	<input type="text" name="help" class="help" value="<?php echo$bily?>">
	<input type="submit" class="salepre"  id="salepre" value="PRE" onclick="this.form.target='_blank';" >
	<input type="submit"  class="salenex" id="salenext" value="NEXT" onclick="f1.action='stnexbil.php';f1.target='_blank';">
	<input type="submit"  class="saledel" id="saledel" value="DELETE" onclick="f1.action='STDE.php';f1.target='_blank';">
</form>
...
<script type="text/javascript">
	...
	c.addEventListener('click',()=>{
		...
		window.location.href="stockopen.php";
	});
</script>
<script>
...
<script type="text/javascript">
	...
	sp.addEventListener('click',()=>
	{
		...
		window.open("sttopbil.php","_blank");
		self.close();
