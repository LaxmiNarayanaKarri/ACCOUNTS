...
mysqli_select_db($con,'books');
	$help= $_POST['help'];
	$bily=$help-1;
	$ss3="select recname from receipt where rebill='$bily'";
	...
	$s1="select MAX(rebill) from receipt";
	$result1=mysqli_query($con,$s1);
	$b1ill=mysqli_fetch_array($result1);
	$max=$b1ill[0];
	$ss1="select MIN(rebill) from receipt";
	$res1=mysqli_query($con,$ss1);
	$lea=mysqli_fetch_array($res1);
	$le=$lea[0];
	$tds="select sum(reamount) as am from receipt where rebill='$bily' ";
...
<div class="salestable"> 
	...
	<tbody class="tbody" id="tbody">
		<?php
			$da="select * from receipt where rebill='$bily' ";
...
<form method="post" action="recpre.php" name="f1" >
	...
	<input type="submit"  class="salenex" id="salenext" value="NEXT" onclick="f1.action='recnexbil.php';f1.target='_blank';">
