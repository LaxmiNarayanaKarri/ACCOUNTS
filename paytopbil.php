...
mysqli_select_db($con,'books');
	$zs="select min(id) from payment";
	$zresult=mysqli_query($con,$zs);
	$zbill=mysqli_fetch_array($zresult);
	$zbil=$zbill[0];
	$s="select pabil from payment where id='$zbil' ";
	$result=mysqli_query($con,$s);
	$bill=mysqli_fetch_array($result);
	$bil=$bill[0];
	$tds="select sum(paamount) as am from payment where pabil='$bil' ";
...
<div class="salestable"> 
	...
	<tbody class="tbody" id="tbody">
		<?php
			$da="select * from payment where pabil='$bil' ";
