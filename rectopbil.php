...
mysqli_select_db($con,'books');
	$zs="select min(id) from receipt";
	$zresult=mysqli_query($con,$zs);
	$zbill=mysqli_fetch_array($zresult);
	$zbil=$zbill[0];
	$s="select rebill from receipt where id='$zbil' ";
	$result=mysqli_query($con,$s);
	$bill=mysqli_fetch_array($result);
	$bil=$bill[0];
	$tds="select sum(reamount) as am from receipt where rebill='$bil' ";
...
<div class="salestable"> 
	...
	<tbody class="tbody" id="tbody">
		<?php
			$da="select * from receipt where rebill='$bil' ";
...
<script type="text/javascript">
	let sp=document.getElementById("salebot");
