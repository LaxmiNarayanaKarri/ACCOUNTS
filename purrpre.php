...
mysqli_select_db($con,'books');
	$help= $_POST['help'];
	$bily=$help-1;
	$ss3="select purrname from purchaser where purrbill='$bily'";
	...
	$ss="select * from purchaser where purrbill='$bily'";
	$res=mysqli_query($con,$ss);
	$re=mysqli_fetch_row($res);
	$s1="select MAX(purrbill) from purchaser";
	...
	$srdat=$re[2];
	$srnar=$re[4];
	$ss1="select MIN(purrbill) from purchaser";
...
<label class="head">PURCHASE RETURN:</label>
<form method="post" action="purchaserphp.php">
	...
	<table class="table" id='table'>
	  	<thread class="thread">
			<tr>
				...
				<th>Amount</th>
		<?php
			$da="select * from purchaser where purrbill='$bily' ";
			$data=mysqli_query($con,$da);
			while($rows=mysqli_fetch_array($data))
			{
				echo "<tr>";
				echo "<td>";
				echo $rows['purrproductname'];
				echo "</td>";
				echo "<td>";
				echo $rows['purrbags'];
				echo "</td>";
...
<form method="post" action="purrpre.php" name="f1" >
	...
	<input type="submit"  class="salenex" id="salenext" value="NEXT" onclick="f1.action='purrnexbil.php';f1.target='_blank';">
	<input type="submit"  class="saledel" id="saledel" value="DELETE" onclick="f1.action='PURRDE.php';f1.target='_blank';">
</form>
...
<script type="text/javascript">
	...
	c.addEventListener('click',()=>{
		...
		window.location.href="purchasereturn.php";
	});
</script>
<script>
...
<script type="text/javascript">
	var least="<?php echo$le ?>";
	var prese="<?php echo$bily ?>";
	if(least==prese)
	{
		document.getElementById("salepre").disabled=true;
	}
</script>
<script type="text/javascript">
	...
	sp.addEventListener('click',()=>
	{
		...
		window.open("purrtopbill.php","_blank");
		self.close();
...
<script type="text/javascript">
	...
	sb.addEventListener('click',()=>
	{
		...
		window.open("purrbot.php","_blank");
		self.close();
