...
mysqli_select_db($con,'books');
	$zs="select MAX(stid) from stock";
	$zresult=mysqli_query($con,$zs);
	$zbill=mysqli_fetch_array($zresult);
	$zbil=$zbill[0];
	$s="select stbill from stock where stid='$zbil' ";
	$result=mysqli_query($con,$s);
	$bill=mysqli_fetch_array($result);
	$bil=$bill[0];
	$ss="select * from stock where stbill='$bil'";
...
<label class="l"> STOCK OPEINIG:</label>
...
<div class="salestable"> 
	...
	<tbody class="tbody" id="tbody">
		<?php
			$da="select * from stock where stbill='$bil' ";
...
<form method="post" action="stpre.php" name="f1" >
	<input type="text" name="help" class="help" value="<?php echo$bil?>">
